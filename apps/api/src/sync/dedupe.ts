...
// 대괄호 말머리 중 이것만 뗀다. SH는 대괄호에 단지명을 넣어([세이지움 태릉입구역] 사용검사 확인증 게시)
// 통째로 떼면 단지만 다른 글들이 한 덩어리로 병합된다
const BRACKET_LABEL = /^(정정공고|정정|재공고|변경공고|재수정|수정|리츠)$/;

/**
 * 병합 키. 정정 계열 말머리와 날짜 괄호만 떼고 나머지 괄호는 남긴다.
 * '완주삼봉(A-1,A-3BL)'처럼 괄호 안이 블록 번호인 경우가 있어 통째로 지우면 다른 공고와 붙는다.
 */
export function mergeKey(title: string): string {
  return title
    .replace(/\[([^\]]*)\]/g, (m, inner: string) => (BRACKET_LABEL.test(inner.replace(/\s+/g, '')) ? ' ' : m))
